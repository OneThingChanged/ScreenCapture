...
- 촬영 후에도 유지되는 크기 조절 프레임을 포함한 영역, 창, 전체 화면 스크린샷
- Windows 시스템 소리와 기본 마이크 입력을 함께 포함하는 영역, 창, 전체 화면 MP4 녹화
- 캡처 해상도와 녹화 비트레이트·인코딩 품질을 각각 선택하는 4단계 프리셋
...
- 이미지 미리보기, 탐색 가능한 영상 재생, 프레임 단위 키보드 이동, 스크롤, 검색, 이름 변경, 삭제, 파일 탐색기에서 보기를 제공하는 관리 탭
...
src/
  ...
  renderer/      캡처·녹화 프레임, 선택 창, 편집기 및 숨김 녹화 렌더러
